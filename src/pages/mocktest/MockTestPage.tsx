  ...
  if (phase === 'intro') {
    return (
      <Card className="max-w-xl mx-auto text-center py-10 space-y-4">
        <h1 className="text-2xl font-bold text-slate-800">Mini Mock Test</h1>
  ...
  if (phase === 'finished') {
    ...
    return (
      <Card className="max-w-xl mx-auto text-center py-10 space-y-3">
        ...
        <h1 className="text-xl font-bold text-slate-800">สรุปผล Mock Test</h1>
        {last ? (
          <>
          ...
          <p className="text-slate-500">หมดเวลาก่อนตอบครบทุกข้อ</p>
        )}
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <div className="flex items-center justify-between">
        <span className="text-sm text-slate-500">ข้อที่ {index + 1}/{total}</span>
      ...
      <Card className="space-y-4">
        ...
        {q.passageOrImage && (
          <div className="rounded-xl bg-slate-100 px-4 py-6 text-center text-sm text-slate-600">{q.passageOrImage}</div>
        )}
        ...
        {q.context && (
          <pre className="whitespace-pre-wrap font-sans text-sm text-slate-700 bg-slate-50 rounded-xl border border-slate-200 px-4 py-3">
            {q.context}
          </pre>
        )}
          ...
          {q.choices.map((choice, i) => (
            <button
              ...
              className={`rounded-xl border px-4 py-3 text-left text-sm transition ${
                selected === i ? 'bg-brand-600 border-brand-600 text-white' : 'bg-white border-slate-200 hover:border-brand-400'
              }`}
            >
              {choice}
            </button>
